...
#ggsave(plot.map.melonly, file="plots/fig1_map_melonly_fpo.pdf", height=7.5, width=7.5)
...
climate<- clim %>% remove_rownames() %>% column_to_rownames(var="shortname") %>%
  select(-c(orch.vin, latitude, longitude, zipcode, weather.station, distance.from.site, day.sample, county.density, month.avg.high.std, month.avg.low.sd, month.avg.avg.std, county, site, X, X.1, X.2, X.3, X.4))
...
clim.pcs<-
  ...
  select(-c(month.avg.high.std, month.avg.low.sd, month.avg.avg.std))
...
examining the differences between coi species assignment across flies  
```{r}
...
```{r}
#load file if want to skip preprocessing 
...
#this is ojbect to use later to save time 
#saveRDS(mel.r, file="latgrad_melR.Rds")
...
Start here for main analyses of fly associated microbiome data 
relative abundance measures
```{r}
#import Rds of preprocessed phyloseq object to save time 
#mel.r <- readRDS(file = "latgrad_melR.Rds")
...
#to test for dispersion differences, making a grouped variable for categorical factors in the above model

#sample type
mel_big_bray_subs<- betadisper(mel_big_bray_df, mel.r.sam$substrate.vis_f)
permutest(mel_big_bray_subs)

#latitude
mel_big_bray_lat<- betadisper(mel_big_bray_df, mel.r.sam$latitude)
#convert group to numeric 
mel_big_bray_lat$group<- as.numeric(mel_big_bray_lat$group)
permutest(mel_big_bray_lat)

#orch.vin
mel_big_bray_ov<- betadisper(mel_big_bray_df, mel.r.sam$orch.vin)
permutest(mel_big_bray_ov)

#climate PC2
mel_big_bray_dim2<- betadisper(mel_big_bray_df, mel.r.sam$Dim.2)

mel_big_bray_dim2$group<- as.numeric(mel_big_bray_dim2$group)

permutest(mel_big_bray_dim2)
...
#betadispersions
#sampletype
mel_big_uni_permu.subs <- betadisper(mel_big_uni_df, mel.r.sam$substrate.vis_f)
permutest(mel_big_uni_permu.subs)

#latitude
mel_big_uni_permu.latitude <- betadisper(mel_big_uni_df, mel.r.sam$latitude)
#convert to numeric for permutest
mel_big_uni_permu.latitude$group <- as.numeric(mel_big_uni_permu.latitude$group)
permutest(mel_big_uni_permu.latitude)


#orch.vin
mel_big_uni_permu.ov <- betadisper(mel_big_uni_df, mel.r.sam$orch.vin)
permutest(mel_big_uni_permu.ov )

#climate pc2
mel_big_uni_permu.dim2 <- betadisper(mel_big_uni_df, mel.r.sam$Dim.2)
#convert to numeric 
mel_big_uni_permu.dim2$group <- as.numeric(mel_big_uni_permu.dim2$group)
permutest(mel_big_uni_permu.dim2)
...
#ggsave(plot.fig.1, file="plots/fig1.pdf", height=6.5, width=9, dpi=600)
#to deal with random white lines in relative abundances 
#ggsave(plot.fig.1, file="plots/fig1_ALL.tiff", height=6.5, width=9, dpi=600)
...
#double check if frass predicts substrate diversity
#not for PD f1,11=1.391, p=0.26
summary(lm(data=m.frs.alpha.m, PD.sb~PD.fr))
#not for Shannon f1,11=0.05, p=0.84
summary(lm(data=m.frs.alpha.m, Sha.sb ~ Sha.fr))

#plot.subfr.pd <- 
ggplot(m.frs.alpha.m, aes(x=PD.fr, y=PD.sb)) + 
  ...
  xlab("Frass PD") + 
  ylab("Substrate PD") +
  ggtitle("A: Subs~Frass PD") +
  theme(legend.position = "right")

#ggsave(plot.subfr.pd, file="v2_plots/supp_subfrasspd_leg.pdf", height=3, width=3)

#plot.subfr.sha<- 
ggplot(m.frs.alpha.m, aes(x=Sha.fr, y=Sha.sb)) + 
  geom_smooth(method="lm", color="black", alpha=0.3) +
  geom_point(aes(color=orch.vin)) +
  theme_classic() +
  scale_color_manual(values=pal.ov) + 
  xlab("Frass Shannon div.") + 
  ylab("Substrate Shannon div.") +
  ggtitle("B: Subs~Frass Shannon") +
  theme(legend.position = "none")

#ggsave(plot.subfr.sha, file="v2_plots/supp_subfrasssha.pdf", height=3, width=3)

#need to look at whether beta diversity was shaped in either substrate or frass
#substrate
subs_bray_df<- phyloseq::distance(m.subs.r, method = "bray")
adonis(subs_bray_df ~ latitude+orch.vin, data = m.subs.r.sam)

#check permutest
#need to join latitude and orchvin
m.subs.r.sam<- 
  m.subs.r.sam %>%
  mutate(permuvars = paste0(latitude, orch.vin)) 

sub_bray_permu <- betadisper(subs_bray_df, m.subs.r.sam$orch.vin)
permutest(sub_bray_permu)

sub_bray_permu_lat<- betadisper(subs_bray_df, m.subs.r.sam$latitude)
permutest(sub_bray_permu_lat)
...
#visualize
#axis 1 = 48.8. axis2=15.5 % explained 
#plot.m.subs.bray<-
#plot.subs.bray.pc<- 
plot_ordination(
    physeq = m.subs.r, 
    ordination = subs.bray, 
    type="samples", 
    justDF=TRUE) %>%
  ggplot(., aes(x=Axis.1, y=Axis.2)) +
    ...
    labs(title="A: Substrate (Bray-Curtis)", fill="Location", x="Axis 1 [48.8%]", y="Axis 2 [15.5%]") +
    theme(legend.position = "none")  
 
#ggsave(plot.subs.bray.pc, file="v2_plots/supp_subsbraypc.pdf", height=3, width=3)


#frass
frass_bray_df<- phyloseq::distance(m.fr.r, method = "bray")
adonis(frass_bray_df ~ latitude+orch.vin, data = m.fr.r.sam)

frass_bray_disp<- betadisper(frass_bray_df, m.fr.r.sam$orch.vin)
permutest(frass_bray_disp)
...
#visualize
#axis 1 = 24.8. axis2=16.7 % explained 
#plot.m.fr.bray.pc<-
plot_ordination(
    physeq = m.fr.r, 
    ordination = fr.bray, 
  ...
  ggplot(., aes(x=Axis.1, y=Axis.2)) +
    ...
    theme_classic() + 
    scale_fill_manual(values=pal.bigstate) +
    labs(title="B: Frass (Bray-Curtis)", fill="Origin", x="Axis 1 [24.8%]", y="Axis 2 [16.7%]") +
    theme(legend.position = "none")  

#ggsave(plot.m.fr.bray.pc, file="v2_plots/supp_frassbraypc.pdf", height=3, width=3)
...
plot_ordination(
    ...
    type="samples", 
    justDF=TRUE) %>%
  ggplot(., aes(x=latitude, y=Axis.2)) +
    geom_smooth(method="lm", color="black", alpha=0.2) +
    geom_point(aes(fill=orch.vin),colour="#636363", pch=21, size=2.5, stroke=0.1) +
    theme_classic() + 
    scale_fill_manual(values=pal.ov) +
    labs(title="C: Frass~Latitude", fill="Origin", x="Latitude", y="Axis 2 [16.7%]") +
    theme(legend.position = "right")  

#ggsave(plot.m.fr.bray.cor, file="v2_plots/supp_frassbraycor.pdf", height=3, width=3)

test.fr.bray<- plot_ordination(
    physeq = m.fr.r, 
    ordination = fr.bray, 
    type="samples", 
    justDF=TRUE)

#axis 1 is not significant, but axis 2 is and  illustrates better than PCoA 
summary(lm(data=test.fr.bray, Axis.1~latitude+orch.vin))

summary(lm(data=test.fr.bray, Axis.2~latitude+orch.vin))
...
plot_ordination(
  ...
  ggplot(., aes(x=Axis.1, y=Axis.2)) +
    ...
    theme_classic() + 
    scale_fill_manual(values=pal.bigstate) +
    scale_color_manual(values=pal.bigstate) + 
    theme(legend.position = "right") + 
...
#test here
adonis(f_bray_df ~latitude + orch.vin + Dim.2 + percMel + shortname, data = lg.fly.r.sam)

#check betadispersion
#latitude
fly_bray_permu.lat <- betadisper(f_bray_df, lg.fly.r.sam$latitude)
#convert latitude to numeric
fly_bray_permu.lat$group <- as.numeric(fly_bray_permu.lat$group)
permutest(fly_bray_permu.lat)

#orch.vin
fly_bray_permu.ov <- betadisper(f_bray_df, lg.fly.r.sam$orch.vin)
permutest(fly_bray_permu.ov)

#clim PC2
fly_bray_permu.dim2 <- betadisper(f_bray_df, lg.fly.r.sam$Dim.2)
#convert latitude to numeric
fly_bray_permu.dim2$group <- as.numeric(fly_bray_permu.dim2$group)
permutest(fly_bray_permu.dim2)

#percent melanogaster
fly_bray_permu.mel <- betadisper(f_bray_df, lg.fly.r.sam$percMel)
#convert latitude to numeric
fly_bray_permu.mel$group <- as.numeric(fly_bray_permu.mel$group)
permutest(fly_bray_permu.mel)

#location
fly_bray_permu.site <- betadisper(f_bray_df, lg.fly.r.sam$shortname)
#convert latitude to numeric
permutest(fly_bray_permu.site)
...
#ggsave(plot.fig.2, file="plots/fig2_ALL.pdf", height=3.25, width=9, dpi=600)
...
#include taxon to see what taxa are above or below
allmel.fit = tyRa::fit_sncm(spp=allmel.otu, pool=NULL, taxon=data.frame(tax_table(lgs.fly.r)))
#default plot from tyRa package 
tyRa::plot_sncm_fit(allmel.fit)

#save the predictions to make a table with summary
#allmel.fit$predictions %>%
#  filter(fit_class != "As predicted") %>%
#  write.table(., file="v2_neutpred.csv", sep=",", row.names = TRUE)
...
#
#ggsave(fig3, file="plots/fig3_ALL.pdf", height=3.25, width=9, dpi=600)
...
#permdisps
#substrate
pair_bray_permu.subs <- betadisper(pair_bray_df, pair.r.sam$substrate.vis_f)
permutest(pair_bray_permu.subs)

#season
pair_bray_permu.season <- betadisper(pair_bray_df, pair.r.sam$season)
permutest(pair_bray_permu.season)

#site
pair_bray_permu.site <- betadisper(pair_bray_df, pair.r.sam$share)
permutest(pair_bray_permu.site)
...
#put together for figure 4
#fig4<-ggarrange(plot.pair.rab, 
...
#ggsave(fig4, file="plots/fig4_ALL.pdf", height=6.5, width=9, dpi=600)
#the saving of this is complicated b/c of quirks with the lines in relative abundance
#ggsave(fig4, file="plots/fig4_ALL.tiff", height=6.5, width=9, dpi=600)

#also have to fix the bray curtis figure colors for some reason

#fig4.bc.leg<-ggarrange(plot.pair.rab, 
#          ggarrange(plot.temp.pair.bray.leg, plot.neut.pair.temporal, nrow=2, labels=c("B", "C")), 
#          ncol=2, labels="A", common.legend=FALSE, widths=c(2.5,2))
#have to frankenstein it back together
#ggsave(fig4.bc.leg, file="plots/fig4_bc_leg.pdf", height=6.5, width=9, dpi=600 )
